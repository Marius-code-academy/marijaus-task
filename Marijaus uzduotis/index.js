...
async function checkHandler() {
  ...
  if (person) {
    ...
    const partyPTextContent = `${person.name.split(' ')[0]} ${person.vip ? 'Is VIP' : 'Is not VIP'}`
    inPartyP.textContent = partyPTextContent;
    container.append(inPartyP);
  }

  const [plusOneCount, countTotalNumber] = await Promise.all([countByParameter('plusOne'), countTotal()]);
  ...
  const memeRes = await fetch('https://test-api-faker.herokuapp.com/cafe04/meme');
